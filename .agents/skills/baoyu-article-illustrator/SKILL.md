...
After every image is generated successfully, upload the local image file with the user's local PicGo configuration before editing any article markdown.

Use this command shape from the project root:

```bash
npx picgo -c "$HOME/Library/Application Support/picgo/data.json" upload <generated-image-path> --format json
```

Parse the returned JSON and use `imgUrl` as the canonical image URL. Keep the local generated file, prompts, and outline as reproducibility/cache artifacts, but article markdown must reference the uploaded `imgUrl`, not the local relative path. If PicGo upload fails, do not insert local image paths as a fallback; report the failure and ask the user how to proceed.
...
11. Upload every successful image with local PicGo per [PicGo Upload Policy](#picgo-upload-policy), capture each returned `imgUrl`, and record the local path → `imgUrl` mapping in `outline.md` or a sidecar note before finalizing markdown.

Full procedures: [references/workflow.md](references/workflow.md#step-5-generate-images)

### Step 6: Finalize

Insert `![description]({picgo-imgUrl})` after paragraphs. The markdown image URL must be the PicGo-uploaded `imgUrl`; do not insert the local relative output path into article markdown.
...
| `default_output_dir` | Local Output Path | Markdown Insert Path |
|----------------------|-------------------|----------------------|
| `imgs-subdir` (default) | `{article-dir}/imgs/` | PicGo uploaded `imgUrl` |
| `same-dir` | `{article-dir}/` | PicGo uploaded `imgUrl` |
| `illustrations-subdir` | `{article-dir}/illustrations/` | PicGo uploaded `imgUrl` |
| `independent` | `illustrations/{topic-slug}/` | PicGo uploaded `imgUrl` |

All auxiliary files (outline) are saved inside the output directory; `prompts/` is transient and removed after verification:
