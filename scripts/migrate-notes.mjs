...
const NOTES_SOURCE_ROOT = "docs/01.知识库";
...
function buildTargetPath(sourceRelPath, title) {
  ...
  const relativeDir = path.relative(NOTES_SOURCE_ROOT, sourceDir);
  ...
  return {
    slug,
    targetRelPath: path.join("notes", ...directoryParts, `${slug}.md`),
    targetAbsPath: path.join(CONTENT_ROOT, "notes", ...directoryParts, `${slug}.md`),
  };
}

async function indexExistingNotesByPermalink() {
...
async function main() {
  ...
  const notesAbsRoot = path.join(sourceRoot, NOTES_SOURCE_ROOT);

  const existingByPermalink = await indexExistingNotesByPermalink();
  const pathCollisionGuard = new Set();

  for (const absPath of existingByPermalink.values()) {
    pathCollisionGuard.add(path.relative(CONTENT_ROOT, absPath));
  }

  const sourceFiles = await listMarkdownFiles(notesAbsRoot);
  const results = [];

  for (const sourceAbsPath of sourceFiles) {
    results.push(await migrateOne(sourceAbsPath, sourceRoot, existingByPermalink, pathCollisionGuard));
  }

  console.log(`Migrated ${results.length} notes from ${notesAbsRoot}`);
  console.log(`Output: ${path.join(REPO_ROOT, "content", "notes")}`);
}
