...
const POSTS_SOURCE_ROOT = "docs/02.工具与部署";
...
function buildTargetPath(sourceRelPath, title) {
  ...
  const relativeDir = path.relative(POSTS_SOURCE_ROOT, sourceDir);
  ...
  return {
    slug,
    targetRelPath: path.join("posts", ...directoryParts, `${slug}.md`),
    targetAbsPath: path.join(CONTENT_ROOT, "posts", ...directoryParts, `${slug}.md`),
  };
}

async function indexExistingPostsByPermalink() {
  const postsRoot = path.join(CONTENT_ROOT, "posts");
...
async function migrateOne(sourceAbsPath, sourceRoot, existingByPermalink, slugCollisionGuard) {
  ...
  // Idempotency: if we already migrated a file with the same source permalink,
  ...
  // Avoid accidental collisions when multiple source files map to same path.
  // Keep the first; suffix the rest.
  if (!isPermalinkOverwrite) {
    let collisionKey = path.relative(CONTENT_ROOT, finalTargetAbsPath);
    if (slugCollisionGuard.has(collisionKey)) {
      ...
      while (true) {
        ...
        if (!slugCollisionGuard.has(candidateKey)) {
    ...
    slugCollisionGuard.add(collisionKey);
  }
  ...
  if (sourcePermalink) {
    existingByPermalink.set(sourcePermalink, finalTargetAbsPath);
  }

  return { sourceRelPath, targetRelPath: path.relative(REPO_ROOT, finalTargetAbsPath) };
}

async function main() {
  const sourceRoot = resolveSourceRoot(process.argv.slice(2), process.env);
  const postsAbsRoot = path.join(sourceRoot, POSTS_SOURCE_ROOT);

  const existingByPermalink = await indexExistingPostsByPermalink();
  const slugCollisionGuard = new Set();

  // Pre-fill collision guard with existing post paths so we don't overwrite them
  // unless we're matching by permalink.
  for (const absPath of existingByPermalink.values()) {
    slugCollisionGuard.add(path.relative(CONTENT_ROOT, absPath));
  }

  const sourceFiles = await listMarkdownFiles(postsAbsRoot);
  const results = [];

  for (const sourceAbsPath of sourceFiles) {
    results.push(await migrateOne(sourceAbsPath, sourceRoot, existingByPermalink, slugCollisionGuard));
  }

  console.log(`Migrated ${results.length} posts from ${postsAbsRoot}`);
  console.log(`Output: ${path.join(REPO_ROOT, "content", "posts")}`);
}
